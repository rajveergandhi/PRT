...
class Utilities():
  ...
  def isDeimos2(self, path):
    ...
    self.isItDeimos2 = False
    tags = self.getTags(path,1)
    retVal = {
              'isDeimos2' : self.isItDeimos2,
              'tags' : tags,
              'path' : path
             }
    return retVal

  def checkDeimos(self, tree):
    for c in tree.findall('Dataset_Sources/Source_Information/Scene_Source/MISSION'):
      str = c.text
      if 'Deimos 2' in str:
          self.isItDeimos2 = True

  def getTags(self, path, check=0):
    # Get tags by parsing the dim file
    tags = list()
    tree = ET.parse(path)

    if check == 1:
      self.checkDeimos(tree)
  ...
  def build(self, itemURI):
    ...
    for img_metadata in tree.findall('Dataset_Sources/Source_Information/Scene_Source'):
      ...
      if viewingAngle is not None:
        metadata['viewingAngle'] = float(viewingAngle.text)
